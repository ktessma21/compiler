...
int tuple_error (int64_t instructionID, int64_t length, int64_t index) {

  /*
   * Check the inputs.
   */
  if ((instructionID & 0x1) != 0x1){
    printf("ERROR: the first input of a call to \"tuple-error (line, length, index)\" is not encoded\n");
    abort();
  }
  if ((length & 0x1) != 0x1){
    printf("ERROR: the second input of a call to \"tuple-error (line, length, index)\" is not encoded\n");
    abort();
  }
  if ((index & 0x1) != 0x1){
    printf("ERROR: the third input of a call to \"tuple-error (line, length, index)\" is not encoded\n");
    abort();
  }
  ...
  printf(" in a tuple that only has %" PRId64 " element", length);
